...
const BoxMesh = ({ faces, faceRotations, dimensions }) => {
  const materials = useMemo(() => {
    const loader = new THREE.TextureLoader();
    const createMaterial = (dataUrl, faceKey) => {
      if (!dataUrl) return new THREE.MeshStandardMaterial({ color: '#ffffff', roughness: 0.9 });
  ...
  const boxArgs = dimensions || [2, 2.5, 1.5];
...
const Box3D = forwardRef(({ faces, faceRotations, dimensions, activePreset, onCameraStateChange }, ref) => {
  const [isActive, setIsActive] = useState(false);
  const controllerRef = useRef();
  const hasSavedCamera = !!localStorage.getItem('camera_state_box');
  ...
  return (
    ...
    >
      <Canvas shadows camera={{ position: [0, 2, 5], fov: 45 }} gl={{ preserveDrawingBuffer: true }}>
        <Stage environment="city" intensity={0.5} adjustCamera={!hasSavedCamera}>
          <BoxMesh faces={faces} faceRotations={faceRotations} dimensions={dimensions} />
        </Stage>
        <CameraController ref={controllerRef} shapeType="box" enableZoom={isActive} activePreset={activePreset} onCameraStateChange={onCameraStateChange} />
      </Canvas>
